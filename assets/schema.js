...
DA.specMeta = function(mode){
  ...
  '    "mode": "' + (mode || "debate") + '",           // debate | ideate | deep（変更しない）',
...
/* kind: debate | ideate | deep ／ rounds: 今回のラウンド数 ／ issues: 今回扱った論点 */
...
DA.normalize = function(input, extra){
  ...
  var rec = {
    ...
    meta: {
      ...
      mode: pick(meta.mode, ["debate","ideate","deep"], pick(extra.mode, ["debate","ideate","deep"], "debate")),
  ...
  rec._sessions = arr(raw._sessions).map(function(s, i){
    ...
    return {
      ...
      kind: pick(s.kind, ["debate","ideate","deep"], "debate"),
...
DA.applyFollowup = function(rec, patch){
  ...
  var kind = pick(sess.kind, ["debate","ideate","deep"], "debate");
